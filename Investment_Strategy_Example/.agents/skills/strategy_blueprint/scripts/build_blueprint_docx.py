...
def _set_run(run, *, size, bold=False, color=BODY, font=FONT_REG):
    run.font.name = font
...
def _add_paragraph(doc_or_cell, text="", *, size=SIZE_BODY, bold=False,
                   color=BODY, align=WD_ALIGN_PARAGRAPH.LEFT,
...
def _add_heading(doc, text, level):
    ...
    Style the run directly rather than using Word's built-in ``Heading 1``
    style. The built-in heading styles carry a theme colour and several
    Office-suite converters (notably LibreOffice's PDF export) treat the
    style colour as authoritative, silently replacing the brand orange.
    Setting font, size and colour on the run is the only reliable way to
    keep ``#E84328`` everywhere.
    """
    spec = {
...
def _add_bullet(doc_or_cell, text, *, level=0):
...
def _border(cell, sides=("top", "left", "bottom", "right"), *, sz=4, color=DIVIDER_HEX):
    tcPr = cell._tc.get_or_add_tcPr()
...
def _cell_text(cell, text, *, size=SIZE_BODY, bold=False, color=BODY,
...
def _set_header_with_mark(section):
...
def _begin_content_section(doc):
...
def render_process_flow(doc, pf):
    # The Process Flow page lives in a fresh section that carries the
...
def _image_or_placeholder(doc, image_path, caption):
...
def _generic_table(doc, headers, rows, col_widths=None):
...
def render_strategy_definition(doc, sd):
    _page_break(doc)
    ...
    for paper in sd.get("background_research", []) or []:
        ...
        if paper.get("link"):
            ...
            _set_run(r2, size=SIZE_BODY, color=RGBColor(0x10, 0x71, 0xE5))
...
def render_feature_engineering(doc, fe):
    _page_break(doc)
...
def render_strategy_design(doc, sdes):
    _page_break(doc)
...
def _render_strategy_score(doc, score):
    ...
    for i, row in enumerate(rows_in, start=1):
        ...
        for j in range(len(headers)):
            ...
            _border(cells[j], sides=("top", "bottom"), sz=4, color=LIGHT_BORDER_HEX)
