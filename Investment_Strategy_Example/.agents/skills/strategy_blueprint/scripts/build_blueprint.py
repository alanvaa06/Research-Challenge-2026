...
S_BODY_BOLD = ParagraphStyle("BodyBold", parent=S_BODY, fontName=FONT_BOLD)
...
S_FOOTER = ParagraphStyle(
    "Footer", parent=S_BODY, fontSize=10, textColor=FOOTER_GREY, alignment=TA_CENTER,
)
...
def _bullets(items, style=S_BULLET):
    return [Paragraph(text, style, bulletText="●") for text in items]


def _numbered(items):
    return [Paragraph(f"{i + 1}. {text}", S_NUMBERED) for i, text in enumerate(items)]


def _placeholder_image(caption, width=6.0 * inch, height=3.0 * inch):
    cell = Paragraph(f"<font color='#7F7F7F'>[{caption} pending]</font>", S_CAPTION)
...
def _image_or_placeholder(path, caption):
...
def _config_table(rows, headers):
...
def _interpolate_color(value, lo, hi):
    if hi == lo:
        return SCORE_MID
...
def _strategy_score_table(score):
...
def render_process_flow(pf):
    flow = [Paragraph("Table of Contents", S_H1)]
    flow.append(Paragraph(f"<b>{pf.get('intro', 'The Process Flow:')}</b>", S_TOC_PROCESS))
...
def render_strategy_definition(sd, missing):
    flow = [Paragraph("Strategy Definition", S_H1)]
    ...
    flow.append(Paragraph(
        sd.get("background_research_intro",
               "Momentum and trend-following strategies have been extensively documented in academic financial research."),
        S_BODY))
    for paper in sd.get("background_research", []) or []:
        ...
        if paper.get("authors"):
            flow.append(Paragraph(f"<b>Authors:</b> {', '.join(paper['authors'])}", S_BODY))
        if paper.get("link"):
            flow.append(Paragraph(
    ...
    if bench.get("secondary"):
        ...
        for key, label in [("fmp_ticker", "FMP Ticker"),
                           ...
                           ("purpose", "Used as")]:
            if s.get(key):
                flow.append(Paragraph(f"     ○ {label}: {s[key]}", S_SUB_BULLET))
...
def render_feature_engineering(fe, missing):
    flow = [Paragraph("Feature Engineering", S_H1)]

    flow.append(Paragraph("Data", S_H2))
    data = fe.get("data", {})
    flow.append(Paragraph(data.get("intro", "The strategy relies primarily on market price data."), S_BODY))
    ...
    for grp in feats.get("groups", []) or []:
        ...
        tbl_data.extend([[r.get("feature", ""), r.get("description", "")] for r in rows])
    ...
    if feats.get("overview_image"):
        flow.extend(_image_or_placeholder(feats["overview_image"], "Features Overview"))
    ...
    for sig in fe.get("strategy_signals", []) or []:
        ...
        flow.extend([Paragraph(f"• {b}", S_BODY) for b in sig.get("logic", []) or []])

    flow.append(PageBreak())
    return flow


def render_strategy_design(sd, missing):
    flow = [Paragraph("Strategy Design", S_H1)]
    ...
    tbl_data.extend([[r.get("dimension", ""), r.get("question", "")] for r in sm_rows])
    ...
    flow.extend([Paragraph(f"• {b}", S_BODY) for b in pc.get("selection_rules", []) or []])
    flow.append(Paragraph("Sizing Rules", S_H3))
    flow.extend([Paragraph(f"• {b}", S_BODY) for b in pc.get("sizing_rules", []) or []])
    ...
    flow.extend([Paragraph(f"• {b}", S_BODY) for b in rb.get("bullets", []) or []])
    ...
    if attr.get("image_path"):
        flow.extend(_image_or_placeholder(attr["image_path"], "Attribution Analysis"))
...
def render_conclusions(cc, missing):
    flow = [Paragraph("Conclusions", S_H1)]
    ...
    for i, group in enumerate(cc.get("next_steps", []) or [], start=1):
        flow.append(Paragraph(f"{i}. <b>{group.get('category', '')}</b>", S_NUMBERED))
...
def render_findings(ff, missing):
    flow = [Paragraph("Findings, Concerns and Decisions", S_H1)]
...
def build(content_path, output_path, strategy_name):
    ...
    score_rows = (
        content.get("strategy_design", {}).get("strategy_score", {}).get("rows", [])
    )
    if score_rows:
