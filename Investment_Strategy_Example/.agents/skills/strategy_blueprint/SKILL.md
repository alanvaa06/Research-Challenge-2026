...
- Leaves the document fully editable — bullets are real bullets (the `●` glyph used in the source PDF), headings are styled directly on the run (no Word "Heading 1/2/3" reference, so the brand orange survives every converter), and the Strategy Score is a real Word table the user can extend.
...
- `references/content_schema.md` — the JSON schema consumed by the build scripts.
