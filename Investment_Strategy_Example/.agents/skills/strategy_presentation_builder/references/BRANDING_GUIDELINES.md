...
These guidelines were extracted from `Investment_Strategy_Example/Strategy Presentation.pdf`. Apply them whenever you build a Strategy Presentation deck so that every output matches the corporate template.
...
- Logo position on content slides: top-right, x ≈ 12.45 in, y ≈ 0.30 in, width ≈ 0.65 in, height ≈ 0.65 in (uses `brand/kaxanuk_logo_red.png`).
...
- Background: white.
- Centered/right-of-center: full KaxaNuk wordmark (`brand/kaxanuk_wordmark.png`) with the gray fingerprint motif (`brand/kaxanuk_fingerprint_gray.png`) anchored to the left.
...
- Background: white with the gray fingerprint motif (`brand/kaxanuk_fingerprint_gray.png`) anchored to the right edge.
- Left-of-center: small `Contact` heading (Montserrat Regular, 32 pt, `#E84328`), with the contact email below it as a hyperlink (Montserrat Regular, 18 pt, `#1155CC`, underlined). The brand red logo (`brand/kaxanuk_logo_red.png`) sits inside the fingerprint motif on the right.
- No page number.
...
- The brand-red logo (`brand/kaxanuk_logo_red.png`) is the only logo used on chrome (top-right of every content slide).
- The wordmark (`brand/kaxanuk_wordmark.png`) is reserved for the cover slide.
- The gray fingerprint motif (`brand/kaxanuk_fingerprint_gray.png`) is reserved for the cover and contact slides.
