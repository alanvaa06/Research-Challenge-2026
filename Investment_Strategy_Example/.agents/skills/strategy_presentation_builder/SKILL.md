...
description: >
  ...
  `Investment_Strategy_Example/Strategy Presentation.pdf`, the brand
...
## Purpose
Render the team's Strategy Presentation as a `.pptx` or `.pdf` that matches the KaxaNuk visual identity (`Investment_Strategy_Example/Strategy Presentation.pdf`) slide-for-slide. Treat the structure as fixed — content varies per strategy, but layouts, titles, and order do not.
...
└── scripts/
    └── build_pptx.py            # python-pptx renderer with --format pptx|pdf
```
...
2. Confirm `Investment_Strategy_Example/Strategy Presentation.pdf` exists. If the user is working in a different folder, ask whether the same template still applies.
...
1. Read `docs/context/memory.md` for locked decisions (universe, signals, portfolio rules, benchmark, backtest window).
...
3. Read `Investment_Strategy_Example/Strategy Scoring.xlsx` (or the project equivalent) for the scoring table to embed on slide 34.
4. Read the most recent `docs/context/session-log.md` entries for any additions since the last deck build.
5. List every gap (missing chart, missing rationale, no decision logged). Surface gaps to the user **before** rendering — do not silently leave slides blank.

## Phase 2 — Strategy Snapshot Interview
Confirm the variables that change per deck. Ask the user to confirm or override defaults from `memory.md`:
...
**PPTX (editable):**
```
python Investment_Strategy_Example/.agents/skills/strategy_presentation_builder/scripts/build_pptx.py \
...
If a verification step fails, edit the JSON or `build_pptx.py` rather than the rendered file. Hand-edits in PowerPoint or a PDF editor break reproducibility, which violates the project's standing guardrails (`docs/context/lessons.md`).

## Phase 6 — Log the Decision
Per `AGENTS.md` §3, every strategic decision must land in `docs/context/memory.md`. After a successful build:

- Log a one-line entry: `YYYY-MM-DD: Built Strategy Presentation v<N> [pptx|pdf] from <commit hash or memory snapshot>; output at Investment_Strategy_Example/Strategy_Presentation.<ext>`.
- If the build surfaced a gap (missing chart, missing rationale), log a numbered rule in `docs/context/lessons.md` so the next iteration does not hit the same gap.
