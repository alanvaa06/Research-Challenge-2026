...
def _run_chunk(identifiers_subset):
    # Worker function: build independent provider instances and run the
    # Data Curator pipeline for the given subset of identifiers.
    local_configurator = _build_configurator()
    base_config = local_configurator.get_configuration()
    sub_config = dataclasses.replace(base_config, identifiers=identifiers_subset)
